...
void entrada(Participante vetor[], int n) {
    cout << "Entrada\n";
    string matricula;
    ...
    if (!jaInscrito(matricula,vetor,n)){
      cout << "Nao existe participante com esta matricula\n";
    return;

    }
    ofstream procuradorArquivo;
    procuradorArquivo.open("entrada.csv", ios::out | ios::app); // Abre o arquivo "entrada.csv" em modo de escrita (ios::out) e para adicionar ao final do arquivo (ios::app).
    procuradorArquivo << matricula <<"\n";
    procuradorArquivo.close();
    cout << "Entrada registrada com sucesso\n";
}

void saida(Participante vetor[], int n) {
    cout << "Saida\n";
    string matricula;
    ...
    ofstream procuradorArquivo;
    procuradorArquivo.open("saida.csv", ios::out | ios::app); // Abre o arquivo "entrada.csv" em modo de escrita (ios::out) e para adicionar ao final do arquivo (ios::app).
    procuradorArquivo << matricula <<"\n";
    procuradorArquivo.close();
